...
t_summary <- data.frame(names = c("Accommodates", "Beds"), p_value = c(round(t1$p.value,2), round(t2$p.value,2)), conf_low = c(round(t1$conf.int[1], 2), round(t2$conf.int[1], 2)), estimate = c(round(t1$estimate,2), round(t2$estimate,2)), conf_high = c(round(t1$conf.int[2], 2), round(t2$conf.int[2],2)))
...
t_data <- data.frame(amenities = c("Washer", "TV", "Familiy / Kid-Friendly", "Dryer", "Elevator in Building", "Lock on Bedroom Door"), P_Value = c(round(t1$p.value, 2), round(t2$p.value,2), round(t3$p.value,2), round(t4$p.value, 2), round(t6$p.value,2), round(t7$p.value,2)), Mean_With = c(round(t1$estimate[1],2), round(t2$estimate[1],2), round(t3$estimate[1],2), round(t4$estimate[1],2), round(t6$estimate[1],2), round(t7$estimate[1],2)), Mean_Without = c(round(t1$estimate[2],2), round(t2$estimate[2],2), round(t3$estimate[2],2), round(t4$estimate[2],2), round(t6$estimate[2],2), round(t7$estimate[2],2)))
