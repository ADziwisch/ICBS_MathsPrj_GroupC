...
map1 <- ggplot() +
  ...
  labs(x = 'Easting', y = 'Northing') + coord_equal() + theme_minimal() +
  scale_colour_gradientn(colours = topo.colors(4), name = "Mean Rent") +
  theme(legend.position = c(.90, .15), legend.box.background = element_rect(fill = "white", colour = "white"))

map2 <- ggplot() +
  geom_polygon(data = lnd_f, aes(long, lat, group = group), fill = "grey95", color = "grey32") +
  geom_point(data = data_short, aes(east, north, color = price_log), alpha = 0.5, size = 1) +
  labs(x = 'Easting', y = 'Northing') + coord_equal() + theme_minimal() +
  scale_colour_gradientn(colours = topo.colors(4), name = "Ln(Price)")  +
  theme(legend.position = c(.90, .15), legend.box.background = element_rect(fill = "white", colour = "white"))
...
levels(cuts$dist_bin) <- gsub(",", " - ", levels(cuts$dist_bin))
...
t_data <- data.frame(amenities = c("Washer", "TV", "Familiy / Kid-Friendly", "Dryer", "Elevator in Building", "Lock on Bedroom Door"), P_Value = c(fp(t1$p.value), fp(t2$p.value), fp(t3$p.value), fp(t4$p.value), fp(t6$p.value), fp(t7$p.value)), Mean_With = c(fp(t1$estimate[1]), fp(t2$estimate[1]), fp(t3$estimate[1]), fp(t4$estimate[1]), fp(t6$estimate[1]), fp(t7$estimate[1])), Mean_Without = c(fp(t1$estimate[2]), fp(t2$estimate[2]), fp(t3$estimate[2]), fp(t4$estimate[2]), fp(t6$estimate[2]), fp(t7$estimate[2])))

t_data <- t_data %>% mutate(Difference = Mean_With - Mean_Without)

colnames(t_data) <- c("Amenity", "P-Value", "Mean Ln(Price) - Included", "Mean Ln(Price) - Excluded", "Difference Ln(Price)")
