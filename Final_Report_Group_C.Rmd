...
```{r echo=FALSE, message=FALSE, warning=FALSE, fig.width=7, fig.height=3.5, fig.cap="Figure 3: Rent Prices vs. Airbnb Prices in London"}
...
t_summary <- data.frame(names = c("Accommodates", "Beds"), p_value = c((t1$p.value), (t2$p.value)), conf_low = c(round(t1$conf.int[1], 2), round(t2$conf.int[1], 2)), estimate = c(round(t1$estimate,2), round(t2$estimate,2)), conf_high = c(round(t1$conf.int[2], 2), round(t2$conf.int[2],2)))
...
Despite the fact that the presented model has obvious limitations regarding factors that could not be quantified, it has direct implications for finding a reasonably priced apartment. Many attributes important for someone searching for a room, like WiFi and the access to a properly equipped kitchen, have small effects on the room price, as they are present in most London apartments. Luxury amenities like the presence of an elevator, a TV and a dryer create costs. It is also good advice to check apartments in less attractive neighbourhoods to save money. Regarding cleanliness, a well-maintained room will be more expensive. Looking at these different attributes of an Airbnb ad, the user is able to determine whether the price of the apartment is actually fair, which was the aim of this report. In order to improve the scope of this paper, the following extension are recommended: Machine learning and picture research methods could provide more objective measures of the attractiveness of the flat, building and neighbourhood. With a broader exploration of the pictures provided in the advertisements, an indicator of room standard could be implemented. 
